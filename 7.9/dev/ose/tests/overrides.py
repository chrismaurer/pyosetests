...
class OSEWaitForFill(WaitForFill):

    def __init__(self, qty=None, fill_type=None, rel_qty=None, op='add', price=None, timeout=120):
        super(OSEWaitForFill, self).__init__(qty, fill_type, rel_qty, op, price, timeout)
        self.leg_feeds = ('order', 'fill')

#This override will not be needed once OSE moves to 7.15
@implements(DeleteOrder)
@scope
def OSEDeleteOrder(order_status=aenums.TT_ORDER_STATUS_OK):
    """
    Send a delete order.
...
@scope
def OSEReplaceOrder(del_order_status=aenums.TT_ORDER_STATUS_OK,
...
ose_wait_for_fill_overrides = Override(OSEWaitForFill)
ose_delete_overrides = Override(OSEDeleteOrder, OSE_HoldOrder_Predicate())
ose_replace_overrides = Override(OSEReplaceOrder, OSE_HoldOrder_Predicate())
OSE_OVERRIDES = [ose_wait_for_fill_overrides, ose_delete_overrides, ose_replace_overrides]
