...
__all__ = ['OSEValidator', 'OSEFillServerValidator', 'OSELogFileValidator',
           'OSELoginValidator', 'OSEPriceValidator', 'OSERFQValidator']

class OSEValidator(FullValidator):
    def __init__(self, log_results=False, throw_results=True):
        ...
        super(OSEValidator, self).__init__(log_results, throw_results)

        # Setup OSE specific validation
...
class OSEFillServerValidator(ValidatorPlugin):
    def __init__(self, log_results=True, throw_results=True):
        self.risk_account_mapped_field = 'exchange_clearing_account'
        super(OSEFillServerValidator, self).__init__(log_results, throw_results)
        setup_fill_server(self)

class OSELogFileValidator(LogValidator):
    def __init__(self, log_results=False, throw_results=True):
        super(OSELogFileValidator, self).__init__(log_results, throw_results)
        setup_file(self.root_table)

class OSELoginValidator(LoginValidator):
    def __init__(self, log_results=True, throw_results=True):
        super(OSELoginValidator, self).__init__(log_results, throw_results)
        setup_login(self.root_table)

class OSEPriceValidator(PriceValidator):
    def __init__(self, log_results=True, throw_results=True):
        super(OSEPriceValidator, self).__init__(log_results, throw_results)
        setup_price_table(self.root_table)

class OSERFQValidator(RFQValidator):
    def __init__(self, log_results=True, throw_results=True):
        super(OSERFQValidator, self).__init__(log_results, throw_results)
