...
def setup_file(table):
    ...
    log_file_table.add_rule(ose_fill_server_start_msg_is_logged, 'ose_fill_server_start_msg_is_logged', cond='False')    
    log_file_table.add_rule(ose_fill_server_stop_msg_is_logged, 'ose_fill_server_stop_msg_is_logged', cond='False')
    
    
     
    log_file_table.override_rule('order_server_state_changed_msg_is_logged', 'True', 160271, note="PCR for Order Server")
    
    log_file_table.optout_rule('fill_server_stop_msg_is_logged', 'is_action_StopGatewayServer and is_server_FillServer', new_rule='ose_fill_server_stop_msg_is_logged', note="Fill Server keeps changing this message" )
    log_file_table.optout_rule('fill_server_start_msg_is_logged', 'is_action_StartGatewayServer and is_server_FillServer', new_rule='ose_fill_server_start_msg_is_logged', note="Fill Server keeps changing this message" )
